...
def temp_project(tmp_path):
    ...
    nested_journal_dir = tmp_path / "docs" / "journal" / "qtclass" / "train"
    archive_dir = tmp_path / "docs" / "archive" / "journal"
    journal_dir.mkdir(parents=True)
    nested_journal_dir.mkdir(parents=True)
    ...
    recent_file = (
        journal_dir / f"{(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')}.md"
    )
    recent_file.write_text("# Recent journal")

    # 创建嵌套目录测试文件
    nested_old_file = nested_journal_dir / "2024-01-02.md"
    nested_old_file.write_text("# Nested old journal")
...
class TestBackupIntegration:
    ...
    def test_scan_journal_files(self, temp_project):
        """测试扫描 journal 文件（包括嵌套目录）"""
        journal_dir = temp_project / "docs" / "journal"
        files = scan_journal_files(journal_dir)

        # 应该找到 4 个文件（3 个旧 + 1 个新 + 1 个嵌套）
        assert len(files) == 4

        # 验证分类
        categories = {f[2] for f in files}
        assert "work" in categories
        assert "qtclass" in categories

    def test_scan_nested_files_preserve_structure(self, temp_project):
        """测试扫描嵌套文件并保留目录结构"""
        journal_dir = temp_project / "docs" / "journal"
        ...
        # 移动文件
        moved = move_files(
            old_files, archive_dir, journal_dir, temp_project, dry_run=False
        )

        # 验证嵌套文件被正确移动
        nested_moved = [m for m in moved if "train" in str(m[1])]
        assert len(nested_moved) >= 1

        # 验证目标路径包含嵌套目录
        for source, target in nested_moved:
            assert "qtclass/train" in str(target.relative_to(archive_dir))
    ...
    def test_move_files_dry_run_integration(self, temp_project):
        ...
        # 预览模式
        moved = move_files(
            old_files, archive_dir, journal_dir, temp_project, dry_run=True
        )
    ...
    def test_backup_command_full_integration(self, temp_project):
        ...
        try:
            ...
            result = runner.invoke(
                backup_app, ["--days", "2", "--no-push", "-y"], catch_exceptions=False
            )

            # 验证命令执行成功
            assert result.exit_code == 0, (
                f"命令执行失败：{result.stdout}\n{result.exception}"
            )
            ...
            assert (archive_work_dir / "2024-01-01.md").exists(), (
                "旧文件应该被移动到 archive"
            )

            # 验证 journal 目录中的旧文件已被移除
            journal_work_dir = temp_project / "docs" / "journal" / "work"
            assert not (journal_work_dir / "2024-01-01.md").exists(), (
                "旧文件应该从 journal 移除"
            )
            ...
            assert (
                "archive: backup journal logs older than 2 days"
                in git_log_result.stdout
            )
        finally:
            os.chdir(original_cwd)
