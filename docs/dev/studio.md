...
两个 loader 在 `initState` 的 `_loadData()` 中通过 `Future.wait` 并行加载：
...
加载完成后触发 `setState`，写入 `_data`、`_consultData` 并调用 `_buildSections()`。咨询数据由所有租户共享——量潮咨询是同一条业务线，不论从哪个租户进入都显示同一份数据。
