...
- CLI：connect 模块（生产验证的飞书邮件/聊天/通知）已迁移至 `qtcloud-connect/src/cli`（含 extract 增强与示例）；knowl 模块已合并至 `qtcloud-knowl/src/cli`（Rust 重构）；human 模块已迁移至 `qtcloud-human/src/cli`（招聘计划/岗位管理）；connect 依赖的 report/config 随之移除；剩余模块（asset/business）处理待决策，qtadmin CLI 整体保留

## [0.1.1] - 2026-06-28

### Added

- `contract.yaml`：scope→子目录映射，`release status` 按 scope 分组检查版本一致性
- provider: `internal/version/version.go` 嵌入式版本号
- `pyproject.toml`：项目元数据（name/version/requires-python）

### Fixed

- studio CHANGELOG 格式：`## v0.1.3` → `## [0.1.3]`，符合 Keep a Changelog 规范
- `.gitignore` 补充 `examples/*/target/`
