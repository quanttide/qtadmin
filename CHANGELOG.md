...
- CLI：connect 模块（生产验证的飞书邮件/聊天/通知）已迁移至 `qtcloud-connect/src/cli`（含 extract 增强与示例）；knowl 模块已合并至 `qtcloud-knowl/src/cli`（Rust 重构）；connect 依赖的 report/config 随之移除；其余模块（asset/business/human）处理待决策，qtadmin CLI 整体保留
