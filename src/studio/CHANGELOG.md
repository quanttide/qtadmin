...
- 评审闭环 MVP：任务分配（角色槽位绑定评审人）、评审工作台（通过/打回留痕）、摩擦登记（复盘四问）、角色页四个治理页面
- 移除写作占位页，导航按执行/制度分组
- 标准库起步：摩擦登记自动生成标准草稿（draft），要素为条件 + 动作 + 证据
- 数据持久化：SharedPreferences 本地 JSON 存储，评审闭环数据不丢

### Test

- AppStore 评审闭环测试：提交/通过/打回/重开状态流转、角色槽位覆盖
- 持久化测试：数据恢复、无历史数据时使用示例数据
- 标准草稿生成测试
...
- 剩余领域包合并回主项目：`data-sources` → `lib/data_sources/`、`qtadmin-navigation` → `lib/navigation.dart`、`qtadmin-org` → `lib/models/` + `lib/screens/` + `lib/views/`，移除 `packages/` 目录
...
- 招聘计划整域下线：删除 human 模型/Screen/数据层（app_state、data_sources），参考实现与契约移至 qtcloud-human 仓库 `examples/recruitment-plan/`，待其 plan 模块承接
- 主项目精简为最小壳：无数据加载、无状态机，路由仅写作占位
