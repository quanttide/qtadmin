...
async fn run(args: UserCommands) {
    ...
    match args {
        UserCommands::List { search, email, limit, skip } => {
            let profiles = if let Some(ref q) = search {
                ...
                .bind(limit).bind(skip).fetch_all(&pool).await.unwrap()
            };
        ...
        UserCommands::Create { real_name, email, phone, school, major, avatar_url, resume_url } => {
            ...
            match result {
                Ok(r) => {
                    let p = sqlx::query_as::<_, UserProfile>("SELECT * FROM user_profiles WHERE id = ?1")
        ...
        UserCommands::Update { id, real_name, email, phone, school, major, avatar_url, resume_url } => {
            let existing = sqlx::query_as::<_, UserProfile>("SELECT * FROM user_profiles WHERE id = ?1")
                .bind(id).fetch_optional(&pool).await.unwrap();
            let existing = match existing {
                Some(p) => p,
                None => { eprintln!("未找到 id={} 的用户", id); return; }
            };
            sqlx::query(
