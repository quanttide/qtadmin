...
// 2. 通过 `knowl::acquire` 用 LLM 从聊天记录中提取课堂政策
...
fn main() -> anyhow::Result<()> {
    ...
    println!("=== 课堂讨论（connect 模块跨群搜索，共 {} 条，展示前 10 条）===\n", msgs.len());
    for m in msgs.iter().take(10) {
        println!("{} | {} | {}", m.time, m.sender, m.content);
    }

    // 2. 通过 knowl 模块用 LLM 提取课堂政策
    let discussion = msgs
        .iter()
    ...
    if entries.is_empty() {
        ...
        let summary_path = output_dir.join("class_summary.md");
