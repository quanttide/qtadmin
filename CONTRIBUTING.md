...
1. 确保代码通过 lint 检查：`dart analyze lib/` 或 `ruff check .`
2. 确保测试通过：`pytest`（provider）或相关测试命令
3. 提交前使用 `cz commit` 生成规范的 commit message
4. PR 标题应概括变更内容，说明中附上变更动机和影响范围
