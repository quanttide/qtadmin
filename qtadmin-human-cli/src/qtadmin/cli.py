...
def mail_list(limit: int, as_json: bool) -> None:
    """List recruitment emails from the inbox.

    Classification results shown here are LOCAL PREVIEWS only.
    The server makes the authoritative classification when ingested.
    """
    cfg = _get_cfg()
    lark = _get_lark(cfg)
    ...
    for i, email in enumerate(emails, 1):
        ...
        click.echo(
            f" {i:>3} │ {email.sender_name:<8} │ {email.subject:<40} │ {status:<14} │ {result.confidence:<6}"
        )
...
def mail_classify(mail_id: str, as_json: bool) -> None:
    """Classify a single email and preview the result (local preview only, not authoritative)."""
    cfg = _get_cfg()
    lark = _get_lark(cfg)
...
def mail_ingest(dry_run: bool, limit: int, status: str | None, with_content: bool, as_json: bool) -> None:
    """Fetch recruitment emails and push raw data to the provider pending queue.

    The CLI sends raw email fields (subject, body, attachments).
    The server makes the authoritative classification decision.
    ...
    for email in emails:
        ...
        for a in attachments:
            raw_attachments.append({
                ...
                "message_attachment_id": a.get("id"),
            })
    ...
    if dry_run or not items:
        ...
        click.echo(f"  ─────────┼─────────────────────────────────┼────────")
        for item in items:
            n_attachments = len(item.get("attachments", []))
            click.echo(f"  {item['sender_name']:<8} │ {item['subject']:<30} │ {n_attachments}")
        if dry_run:
    ...
    _eprint(f"  Data is now in the pending queue. Confirm via API or studio.")
...
def mail_send(loop: bool, interval: int) -> None:
    """Send pending outbound messages from the outbox.

    Claims pending messages from the server and sends them via lark-cli.
    Use --loop for continuous polling (daemon mode).
    """
    cfg = _get_cfg()
    api = _get_api(cfg)
    lark = _get_lark(cfg)
    ...
    if loop:
        ...
        while True:
            try:
                n = send_pending(api, lark)
                if n:
                    _eprint(f"Sent {n} messages this cycle")
            except Exception as e:
                _eprint(f"Send cycle failed: {e}")
            time.sleep(interval)

    n = send_pending(api, lark)
    _eprint(f"Sent {n} messages")
...
class StatusGroup(click.MultiCommand):
    """Custom group for 'human status' subcommands."""
...
def _status_pending(as_json: bool) -> None:
    """Show counts of pending, confirmed, and ignored queue items."""
    cfg = _get_cfg()
    api = _get_api(cfg)
...
def _status_last_ingest() -> None:
    """Show result of the most recent ingest operation."""
