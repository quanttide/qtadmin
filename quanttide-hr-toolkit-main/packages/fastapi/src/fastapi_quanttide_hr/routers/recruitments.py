...
from fastapi_quanttide_hr.models.talent import ALLOWED_STATUSES_FOR_SUB_STAGE, Talent, TalentStatus
...
from fastapi_quanttide_hr.schemas.talent import SubStageUpdate, TalentCreate, TalentRead, TalentTransition, TalentUpdate
from fastapi_quanttide_hr.schemas.recruitment import HeadcountRead, RecruitmentCreate, RecruitmentRead
from fastapi_quanttide_hr.services.headcount import get_headcount
...
def create_talent(recruitment_id: int, data: TalentCreate, db: Session = Depends(get_db)):
    ...
    # Phase 2: Candidate + Application are primary write targets
    ...
    # Talent derived from Application
...
def set_talent_sub_stage(recruitment_id: int, talent_id: int, data: SubStageUpdate, db: Session = Depends(get_db)):
    ...
    if t.application:
        t.application.sub_stage = data.sub_stage
    db.commit()
    db.refresh(t)
    return t
